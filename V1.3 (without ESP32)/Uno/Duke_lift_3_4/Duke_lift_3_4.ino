...
// ---------- Node Identity ----------
const char NODE_TAG[] = "node2";
const char LIFT_A[]   = "lift3";
const char LIFT_B[]   = "lift4";
...
void loop() {
  ...
  // ---------- Lift 3 (state change only) ----------
  if (curA != lastA) {
    lastA = curA;
    sendLiftState(LIFT_A, (int)curA);
    Serial.print(F("Lift3: "));
    Serial.println(curA ? F("ON") : F("OFF"));
  }

  // ---------- Lift 4 (state change only) ----------
  if (curB != lastB) {
    lastB = curB;
    sendLiftState(LIFT_B, (int)curB);
    Serial.print(F("Lift4: "));
